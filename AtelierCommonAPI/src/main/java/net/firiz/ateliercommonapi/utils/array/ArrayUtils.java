package net.firiz.ateliercommonapi.utils.array;
...
public class ArrayUtils {
    ...
    public static <T> T[] add(T[] array, T object) {
        final T[] next = Arrays.copyOf(array, array.length + 1);
        next[next.length - 1] = object;
        return next;
    }

    @SafeVarargs
    public static <T> T[] addAll(T[] array, T... objects) {
        final int oldLength = array.length;
        final int len = objects.length;
        final T[] next = Arrays.copyOf(array, oldLength + len);
        System.arraycopy(objects, 0, next, oldLength, len);
        return next;
    }
